...
def find_corresponding_lesions_timepoints(df1_file, df2_file, threshold):
    """
    Find corresponding lesions between two sets of lesions.

    Parameters:
        df1_file (str): The file path to the first set of lesions.
        df2_file (str): The file path to the second set of lesions.
        threshold (float): The threshold distance for considering lesions as corresponding.
...
def process_lesion_timepoints(screening_file, transformed_file, threshold):
    """
    Process lesion correspondences between two timepoints.

    Parameters:
        screening_file (str): The file path to the screening timepoint data.
        transformed_file (str): The file path to the transformed timepoint data.
        threshold (float): The threshold distance for considering lesions as corresponding.

    Returns:
        Tuple[pandas.DataFrame, pandas.DataFrame, pandas.DataFrame]: 
        A tuple containing correspondences DataFrame, unmatched indices DataFrame from the screening file,
        and unmatched indices DataFrame from the transformed file.
    """
...
def create_final_dataframe_timepoints(correspondences, unmatched_names_df1, unmatched_names_df2, df1_file, df2_file):
    ...
    Parameters:
        correspondences (pandas.DataFrame): DataFrame containing lesion correspondences.
